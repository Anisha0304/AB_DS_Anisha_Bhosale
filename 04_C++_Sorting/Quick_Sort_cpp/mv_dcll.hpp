...
MV_DCLL(MV_DCLL& list, long first, long last) 
{
...
mp_tail = mp_head;
m_size = 0;
Node<T_LIST>* prunner = list.mp_head->mp_next;
while( prunner != list.mp_head )
{
InsertBack(prunner->m_data);
prunner = prunner->mp_next;
}
...
{
private:
Node<T_LIST>* node_itr;
public:
Iterator(Node<T_LIST>* itr) 
{
node_itr = itr; 
}
~Iterator() 
...
//std::cout << "Itr Destructor" << std::endl;
...
Iterator temp = Iterator(node_itr);
++(*this);
return(temp);
}
Iterator operator--(int value) 
...
{
return( this->node_itr == iterator.node_itr );
}
bool operator!=(Iterator& iterator)
{
...
return(node_itr->m_data);
}
Node<T_LIST>* operator!()
{
...
return( Iterator(this->mp_head->mp_next) );
}
Iterator end()
{
