...
static ret_t error_checking(const int assert_condition,const char* assert_condition_str,const char* file_name,const long line_no,const int error_condition,const char* error_str);

//Macros 

#define ERROR_CONDITION(assert_condition,error_condition,error_str) (									\
...
static ret_t error_checking(const int assert_condition,const char* assert_condition_str,const char* file_name,const long line_no,const int error_condition,const char* error_str)
{
	...
	if(!assert_condition)
	{
		fprintf(stdout,"Assertion Filed: %s,File:%s,Line #:%ld\n",assert_condition_str,file_name,line_no);
		return(FAILURE);
	}
	#else
...
extern p_mv_stack_t create_stack(void)
{
	...
	ERROR_CHECKING(NULL !=pstack,1,NULL);
	pstack->pvector=create_vector();
	ERROR_CHECKING(NULL !=pstack->pvector,1,NULL);
	return(pstack);
}
extern ret_t mv_stack_push(p_mv_stack_t pstack,data_t data)
{
	ERROR_CHECKING(NULL !=pstack,NULL ==pstack,"ERROR:Stack not found.\n");
...
{
	ret_t status=ERROR_CHECKING(NULL !=pstack,NULL ==pstack,"ERROR:Stack not found.\n");
	if(FAILURE ==status)
		return;
	status=ERROR_CHECKING(NULL !=pPrintDataFunc,NULL ==pPrintDataFunc,"ERROR:Print Function Not Found.\n");
	if(FAILURE == status)
		return;
	fprintf(stdout,"\n STACK:\n");
	fprintf(stdout,"TOP\n |\n");
...
extern ret_t mv_stack_destroy(pp_mv_stack_t ppstack)
{
	...
	mv_stack_destroy(&(pstack->pvector));
	if(pstack)
	{
		free(pstack)
		pstack=NULL;
