...
static int balance_factor(p_tree_node_t pnode)
{

//Code

int left_height =height(pnode->left);
int right_height =height(pnode->right);
return(left_height - right_height);
}
static p_tree_node_t rotate(p_tree_node_t pnode)
{

//Code

int balancefactor =balance_factor(pnode);
if(balancefactor < -1)
{
if(NULL ==pnode->right->right)
right_rotate(pnode->right);
return(left_rotate(pnode));
}
else if(1 < balancefactor)
{
if(NULL ==pnode->left->left)
...
p_tree_node_t temp =prunner->parent;
...
fprintf(stdout,"-");
fprintf(stdout,"%d",balance_factor(prunner));
fprintf(stdout,"}->");
if(prunner->left)
prunner =prunner->left;
else
 ...
 * Input:  None
 * Returns: pointer to struct mv_avl object
 * 
 */
